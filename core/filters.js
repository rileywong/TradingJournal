 ...
 * @param {string} [criteria.setup]   trade's setup/strategy (exact)
...
export function filterTrades(trades, criteria = {}) {
  ...
  const setup = criteria.setup ? String(criteria.setup).trim() : null;
  ...
  return trades.filter((t) => {
    ...
    if (setup && String(t.setup || '').trim() !== setup) return false;
...
/** Distinct, sorted setups present across a trade list (for filter dropdowns). */
export function distinctSetups(trades) {
  const set = new Set();
